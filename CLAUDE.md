...
> **Публичный/приватный репозиторий.** Часть документации живёт в `private/` — не публикуется, содержит нереализованные требования, roadmap 1.1+/1.2, открытые вопросы (включая монетизацию OQ-01) и внутренний контекст решений. `docs/` — публичная витрина (README, реализованные ✅-требования, архитектура безопасности, схемы данных). Для внутренней работы (включая решение открытых вопросов, планирование фич вне 1.0) всегда используй полные версии из `private/`, не публичные из `docs/`. Подробности разделения — `docs/agent/CONTEXT.md` (после первой фиксации решения) или спроси разработчика.
...
| `Development_Roadmap.md` | `private/Development_Roadmap.md` | Сомнение «это в 1.0 или позже» — сверить со scope (раздел 6). |
...
| `Local_LLM_Spec.md` | `private/Local_LLM_Spec.md` | **Только справочно для 1.2.** В коде 1.0 не реализовывать. |
...
Single-context layout, but at a non-default path: `docs/agent/CONTEXT.md` (glossary + decision rationale only — `private/TZ.md` stays the requirements/ID registry) and `docs/agent/adr/`. See `docs/agent/domain.md`.
