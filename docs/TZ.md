...
| ✅ SSH-05 | *(mark restored 2026-08-04 — the second SSH hop is now actually established and covered by tests, verified live against a real server)* Jump host (ProxyJump) and a custom port are supported. Settings are imported from ~/.ssh/config. | Should | SSH-01 | Connecting through a jump host from an imported config. |
